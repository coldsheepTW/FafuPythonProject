...
def push_track_delay(url, audio_data, samplerate, instance_name, cs):
...
def push_empty(url, samplerate, instance_name):
    chunk_size = samplerate * 20  # ADJUST
...
def push_empty_chunks(url, samplerate, instance_name, cs):
    chunk_size = samplerate  # ADJUST
    sleep_between_chunks = 0.7  # ADJUST
    block_until_playback_is_finished = True  # ADJUST
    ...
    with grpc.insecure_channel(url) as channel:
        ...
        # Then we send messages with audio_data
            ...
            #while cs.listening:
            while cs.listening or cs.thinking:
...
def push_audio_chunks(url, audio_data, samplerate, instance_name):

    chunk_size = samplerate // 10  # ADJUST
    sleep_between_chunks = 0.07  # ADJUST
